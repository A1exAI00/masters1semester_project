...
NEC1S1763_5segm_approx.display_params(obl8)
# println(obl8.is_λ_real)
# println((obl8.u_st, obl8.v_st))
# println(obl8.C₁(1.0,0.5))
# println(obl8.C₂(1.0,0.5))

u₀ = obl8.u_max_bound
u_L_end = obl8.u_min_bound

#########################################################################################

function L_calc_τ(v₀)
    u(t) = obl8.sol_u(obl8.C₁(u₀,v₀), obl8.C₂(u₀,v₀), t)
    v(t) = obl8.sol_v(obl8.C₁(u₀,v₀), obl8.C₂(u₀,v₀), t)
    f(τ,p) = u(τ) - u_L_end
...
savedir = joinpath("tmp", "21-5_segm_S7-S8_map")
savepath = plotsdir(savedir, "21-5_segm_S7-S8_map_$(time_ns()).pdf")
savefig(p, savepath)