...
v₀_start, v₀_end, v₀_N = 1.0, 1.7, 10
v₀_range = range(v₀_start, v₀_end, v₀_N)


#########################################################################################

# k₂ = R₀*(NEC1S1763_5segm_approx.I_2-NEC1S1763_5segm_approx.I_1)/(NEC1S1763_5segm_approx.V_2-NEC1S1763_5segm_approx.V_1)
# b₂ = -(k₂-1.0)

# obl2 = NEC1S1763_5segm_approx.obl(2, 1.0, u_L_end, k₂, b₂, δ, E, ε)
obl2 = NEC1S1763_5segm_approx.create_obl_from_obl_N(2, δ, E, ε)

NEC1S1763_5segm_approx.display_params(obl2)
# println(obl2.is_λ_real)
# println((obl2.u_st, obl2.v_st))
# println(obl2.C₁(1.0,0.5))
# println(obl2.C₂(1.0,0.5))

u₀ = obl2.u_min_bound
u_L_end = obl2.u_max_bound

#########################################################################################

function L_calc_τ(v₀)
    u(t) = obl2.sol_u(obl2.C₁(u₀,v₀), obl2.C₂(u₀,v₀), t)
    v(t) = obl2.sol_v(obl2.C₁(u₀,v₀), obl2.C₂(u₀,v₀), t)
    f(τ,p) = u(τ) - u_L_end
...
# plot!(p1, [obl2.u_st], [obl2.v_st], seriestype=:scatter, label=nothing)


p = plot(p1, legend=false, size=(500,400), dpi=300)

savedir = joinpath("tmp", "15-5_segm_S1-S2_map")
savepath = plotsdir(savedir, "15-5_segm_S1-S2_map_$(time_ns()).pdf")
savefig(p, savepath)