...
NEC1S1763_5segm_approx.display_params(obl5)
# println(obl5.is_λ_real)
# println((obl5.u_st, obl5.v_st))
# println(obl5.C₁(1.0,0.5))
# println(obl5.C₂(1.0,0.5))

u₀ = obl5.u_min_bound
u_L_end = obl5.u_min_bound

#########################################################################################

function L_calc_τ(v₀)
    u(t) = obl5.sol_u(obl5.C₁(u₀,v₀), obl5.C₂(u₀,v₀), t)
    v(t) = obl5.sol_v(obl5.C₁(u₀,v₀), obl5.C₂(u₀,v₀), t)
    f(τ,p) = u(τ) - u_L_end
...
plot!(p1, [obl5.u_st], [obl5.v_st], seriestype=:scatter, label=nothing)


p = plot(p1, legend=false, size=(500,400), dpi=300)

savedir = joinpath("tmp", "18-5_segm_S4-S5_map")
savepath = plotsdir(savedir, "18-5_segm_S4-S5_map_$(time_ns()).pdf")
savefig(p, savepath)