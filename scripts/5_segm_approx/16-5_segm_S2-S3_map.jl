...
u₀ = NEC1S1763_5segm_approx.V_2/NEC1S1763_5segm_approx.V_1
u_L_end = NEC1S1763_5segm_approx.V_3/NEC1S1763_5segm_approx.V_1

#########################################################################################

obl3 = NEC1S1763_5segm_approx.create_obl_from_obl_N(3, δ, E, ε)

NEC1S1763_5segm_approx.display_params(obl3)
# println(obl3.is_λ_real)
# println((obl3.u_st, obl3.v_st))
# println(obl3.C₁(1.0,0.5))
# println(obl3.C₂(1.0,0.5))

u₀ = obl3.u_min_bound
u_L_end = obl3.u_max_bound

#########################################################################################

function L_calc_τ(v₀)
    u(t) = obl3.sol_u(obl3.C₁(u₀,v₀), obl3.C₂(u₀,v₀), t)
    v(t) = obl3.sol_v(obl3.C₁(u₀,v₀), obl3.C₂(u₀,v₀), t)
    f(τ,p) = u(τ) - u_L_end
    sol = solve(IntervalNonlinearProblem(f, (0.0, 0.25)))
    τ₁ = sol.u
    return τ₁
end
...
plot!(p1, [obl3.u_st], [obl3.v_st], seriestype=:scatter, label=nothing)


p = plot(p1, legend=false, size=(500,400), dpi=300)

savedir = joinpath("tmp", "16-5_segm_S2-S3_map")
savepath = plotsdir(savedir, "16-5_segm_S2-S3_map_$(time_ns()).pdf")
savefig(p, savepath)